...
/**
 * Create transformed arrayay based on the control sequences that original
 * arrayay contains
 *
 * @param {arrayay} array initial arrayay
 * @returns {arrayay} transformed arrayay
 *
 * @example
 *
...
function transform(arr) {
  ...
  for (let i = 0; i < a.length; i++) {
    if (
      controlNext.includes(a[i]) &&
      i < a.length - 1 &&
      !control.includes(a[i + 1])
    ) {
      a[i] == controlNext[0]
        ? a.splice(i + 1, 1)
